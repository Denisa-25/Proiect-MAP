...
class Server:
    ...
    def genereaza_serviciu_si_plecare(self, client_id, current_time, service_rate):
...
class Simulator:
    ...
    def proceseaza_plecare(self, event: Event):
        ...
        self.wait_times.append(self.current_time - start_time)
    ...
    def programeaza_urmatoarea_sosire(self):
        timp_viitor = self.current_time + random.expovariate(self.arrival_rate)
        if timp_viitor <= self.max_time:
            eveniment_sosire = Event(
                time=timp_viitor,
                priority=0,
                type="arrival",
    ...
    def run(self):
        ...
        while self.event_queue and self.current_time <= self.max_time:
            ...
            if event.type == "arrival":
                self.proceseaza_sosire(event)
                self.programeaza_urmatoarea_sosire()
            elif event.type == "departure":
                self.proceseaza_plecare(event)

        if self.wait_times:
            print("Timp mediu de asteptare:", sum(self.wait_times) / len(self.wait_times))
        else:
            print("Nimeni nu a fost servit")

sim = Simulator(num_servers=2, arrival_rate=0.5, service_rate=1.0, max_time=50)
sim.run()
